 ...
 * @license				http://www.jxuncall.com/license/
 * @lastmodify			2010-6-1
 */
...
function L($language = 'no_language',$pars = array(), $modules = '') {
	...
	if(!$LANG) {
		require_once WEB_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system.lang.php';
		if(defined('IN_ADMIN')) require_once WEB_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.'system_menu.lang.php';
		if(file_exists(WEB_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php')) require WEB_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.ROUTE_M.'.lang.php';
	}
	if(!empty($modules)) {
		$modules = explode(',',$modules);
		foreach($modules AS $m) {
			if(!isset($LANG_MODULES[$m])) require WEB_PATH.'languages'.DIRECTORY_SEPARATOR.$lang.DIRECTORY_SEPARATOR.$m.'.lang.php';
		}
	}
...
function getcache_common(){
	$file_content=getcache('common', 'commons');
	...
	//如果座席监控分机列表地址为相对路径，则完整路径动态获取
	if(strpos($file_content['extension_url'], 'http:')===false){
		$file_content['extension_url']='http://'.$_SERVER['HTTP_HOST'].$file_content['extension_url'];
	}
	//如果录音播放接口地址为相对路径，则完整路径动态获取
	if(strpos($file_content['record_url'], 'http:')===false){
		$file_content['record_url']='http://'.$_SERVER['HTTP_HOST'].$file_content['record_url'];
	}
	if(strpos($file_content['agent_statistic'], 'http:')===false){
		$file_content['agent_statistic']='http://'.$_SERVER['HTTP_HOST'].$file_content['agent_statistic'];
	}
	if(strpos($file_content['queue_statistic'], 'http:')===false){
		$file_content['queue_statistic']='http://'.$_SERVER['HTTP_HOST'].$file_content['queue_statistic'];
	}
	return $file_content;
}
...
function menu_linkage($linkageid = 0, $id = 'linkid', $defaultvalue = 0) {
	...
	if($datas['style']=='1') {
		...
		if(!defined('DIALOG_INIT_1')) {
			...
			/*
			//$string .= '<script type="text/javascript" src="'.JS_URL.'dialog.js"></script>';
			//TODO $string .= '<link href="'.CSS_PATH.'dialog.css" rel="stylesheet" type="text/css">';
			*/
		}
		if(!defined('LINKAGE_INIT_1')) {
			define('LINKAGE_INIT_1', 1);
			$string .= '<script type="text/javascript" src="'.JS_URL.'linkage/js/pop.js"></script>';
		}
		...
		if(!defined('LINKAGE_INIT')) {
			...
			$string .= '<script type="text/javascript" src="'.JS_URL.'linkage/js/mln.colselect.js"></script>';
			if(defined('IN_ADMIN')) {
				$string .= '<link href="'.JS_URL.'linkage/style/admin.css" rel="stylesheet" type="text/css">';
			} else {
				$string .= '<link href="'.JS_URL.'linkage/style/css.css" rel="stylesheet" type="text/css">';
			}
		}
...
function runhook($method) {
	...
	if(!empty($hook_appid)) {
		foreach($hook_appid as $appid => $p) {
			$pluginfilepath = WEB_PATH.'plugin'.DIRECTORY_SEPARATOR.$p.DIRECTORY_SEPARATOR.'hook.class.php';
			$getpclass = TRUE;		
...
function admin_tpl($file, $m = '') {
	...
	return WEB_PATH.'modules'.DIRECTORY_SEPARATOR.$m.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.$file.'.tpl.php';
}
//获取配置文件
function get_config($file, $key='') {
