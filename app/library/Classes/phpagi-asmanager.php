  ...
  class AGI_AsteriskManager
  {
    ...
    */
    function AGI_AsteriskManager($config=NULL, $optconfig=array())
    {
      ...
      if(!isset($this->config['asmanager']['port'])) $this->config['asmanager']['port'] = 6048;
    ...
    */
    function wait_response($allow_timeout=false)
    {
      ...
      } while($type != 'response' && $type != 'proxyresponse' && !$timeout);
	  if($type=='proxyresponse'){
		$parameters['data']='';
		$parameters['Response']=$parameters['ProxyResponse'];
		unset($parameters['ProxyResponse']);
		if(isset($parameters['Message']))unset($parameters['Message']);
	  }
      return $parameters;
    }
