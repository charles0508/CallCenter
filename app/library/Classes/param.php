 ...
 * @license				http://www.singhead.com/license/
 * @lastmodify			2010-6-7
 */
class param {
	//路由配置
	...
	public function __construct() {
		...
		$this->route_config = pc_base::load_config('route', 'default');
	...
	public function route_a() {
		...
		if (empty($a)) {
			$a = isset($_GET['action']) && !empty($_GET['action']) ? $_GET['action'] : (isset($_POST['action']) && !empty($_POST['action']) ? $_POST['action'] : '');
		}
		if (empty($a)) {
