...
class form {
	...
	//选择用户
	public function selectuser($id = 'userid', $str = '', $defaultvalue = '', $btnname='选择用户'){
		$id=$id?$id:'id'.uniqid();
		$html='<script type="text/javascript">';
		$html.='function showModalDlg(){';
		$html.='var obj = new Object();';
		$html.='var str=window.showModalDialog("?m=admin&c=admin_manage&a=public_selectuser&defaultval="+$("#'.$id.'").val()+"';
		$html.='&menuid='.$_GET['menuid'];
		$html.='&pc_hash='.$_SESSION['pc_hash'].'",obj,"dialogWidth=400px;dialogHeight=250px");';
		$html.='$("#'.$id.'").val(str);';
		$html.='}';
		$html.='</script>';
		$html.='<input type="hidden" id="'.$id.'" '.$str.' value="'.$defaultvalue.'"/>';
		$html.='<input type="button" class="button" value="'.$btnname.'" onclick="showModalDlg()"/>';
		return $html;
	}
