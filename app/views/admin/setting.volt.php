<?= $this->getContent() ?>
<?= $this->tag->javascriptInclude('public/js/jquery.form.js') ?>
...
<!--系统通用配置-->
  ...
  <tr>
    ...
    <td><select name="info[mixpopscreen][mix_debug]">
      <?php $yessel = ($config['mixpopscreen']['mix_debug'] == 'YES' ? 'selected' : ''); ?>
      <?php $nosel = ($config['mixpopscreen']['mix_debug'] == 'NO' ? 'selected' : ''); ?>
  ...
  <tr>
    ...
    <td><select name="info[mixpopscreen][pop_type]">
      <?php $ringsel = ($config['mixpopscreen']['pop_type'] == 'RING' ? 'selected' : ''); ?>
      <?php $linksel = ($config['mixpopscreen']['pop_type'] == 'LINK' ? 'selected' : ''); ?>
...
<!--   <?php $localsel = ($config['common']['runmode'] == 'local' ? 'selected' : ''); ?>
  <?php $remotesel = ($config['common']['runmode'] == 'remote' ? 'selected' : ''); ?> -->
  <!-- <tr>
    <th>接口运行模式：</th>
  ...
  <tr>
    <th>心跳机制：</th>
    <td><?php $this->_macros['overtime'] = function($__p = null) { if (isset($__p[0])) { $time = $__p[0]; } else { if (isset($__p["time"])) { $time = $__p["time"]; } else {  throw new \Phalcon\Mvc\View\Exception("Macro 'overtime' was called without parameter: time");  } }  ?>
          <?php if ($time >= 60) { ?>
            <?php $display_time = $time / 60 . '分钟'; ?>
          <?php } else { ?>
            <?php $display_time = $time . '秒'; ?>
          <?php } ?>
          <?= $display_time ?><?php }; $this->_macros['overtime'] = \Closure::bind($this->_macros['overtime'], $this); ?>
      <select name="info[common][ping]">
        <?php $times = [0, 10, 20, 30, 40, 50, 60, 120, 300, 600, 1200, 1800]; ?>
        <?php foreach ($times as $time) { ?>
          <?php if ($config['common']['ping'] == $time) { ?>
            <?php $timesel = 'selected'; ?>
          <?php } else { ?>
            <?php $timesel = ''; ?>
          <?php } ?>
          <option value="<?= $time ?>" <?= $timesel ?>><?= $this->callMacro('overtime', [$time]) ?></option>
        <?php } ?>
      </select>
    </td>
...
<!--控制面板配置-->
  ...
  <tr>
    <th>示忙类型：</th>
    <td><textarea name="info[mixpanel][busytype]" rows="3"><?= trim($config['mixpanel']['busytype']) ?></textarea></td>
...
<!--   <tr class="head">
  ...
  <tr>
    ...
    <td><select name="info[mixpopscreen][pop_mode]">
      <?php $flashsel = ($config['mixpopscreen']['pop_mode'] == 'flash' ? 'selected' : ''); ?>
      <?php $html5sel = ($config['mixpopscreen']['pop_mode'] == 'html5' ? 'selected' : ''); ?>
...
<!--   <tr>
    ...
    <td><select name="info[mixpopscreen][pop_up_when_dial_in]">
      <?php $dial_in_yes_sel = ($config['mixpopscreen']['pop_up_when_dial_in'] == '1' ? 'selected' : ''); ?>
      <?php $dial_in_no_sel = ($config['mixpopscreen']['pop_up_when_dial_in'] == '0' ? 'selected' : ''); ?>
  ...
  <tr>
    ...
    <td><select name="info[mixpopscreen][pop_up_when_dial_out]">
      <?php $dial_out_yes_sel = ($config['mixpopscreen']['pop_up_when_dial_out'] == '1' ? 'selected' : ''); ?>
      <?php $dial_out_no_sel = ($config['mixpopscreen']['pop_up_when_dial_out'] == '0' ? 'selected' : ''); ?>
  ...
  <tr>
    ...
    <td><select name="info[mixpopscreen][open_type]" onChange="open_type_display(this.value)">
      <?php $open_type_sel1 = ($config['mixpopscreen']['open_type'] == '1' ? 'selected' : ''); ?>
      <?php $open_type_sel2 = ($config['mixpopscreen']['open_type'] == '2' ? 'selected' : ''); ?>
  ...
  </tr> --><?php $this->_macros['create_time_select'] = function($__p = null) { if (isset($__p[0])) { $name = $__p[0]; } else { if (isset($__p["name"])) { $name = $__p["name"]; } else {  throw new \Phalcon\Mvc\View\Exception("Macro 'create_time_select' was called without parameter: name");  } } if (isset($__p[1])) { $currenttime = $__p[1]; } else { if (isset($__p["currenttime"])) { $currenttime = $__p["currenttime"]; } else {  throw new \Phalcon\Mvc\View\Exception("Macro 'create_time_select' was called without parameter: currenttime");  } } if (isset($__p[2])) { $timeArray = $__p[2]; } else { if (isset($__p["timeArray"])) { $timeArray = $__p["timeArray"]; } else {  throw new \Phalcon\Mvc\View\Exception("Macro 'create_time_select' was called without parameter: timeArray");  } }  ?>
    <select name="<?= $name ?>">
      <?php foreach ($timeArray as $key => $time) { ?>
      ...
      <?php } ?>
    </select><?php }; $this->_macros['create_time_select'] = \Closure::bind($this->_macros['create_time_select'], $this); ?>
...
<!--洗号系统配置-->
  ...
  <tr>
  <th>启动时间：</th> 
  <td>年:<?= $this->callMacro('create_time_select', ['gsm[starttime][year_start]', $config['time_start']['year_start'], $config['time_array']['yearArray']]) ?>
    月:<?= $this->callMacro('create_time_select', ['gsm[starttime][month_start]', $config['time_start']['month_start'], $config['time_array']['monthArray']]) ?>
    日:<?= $this->callMacro('create_time_select', ['gsm[starttime][mday_start]', $config['time_start']['mday_start'], $config['time_array']['mdayArray']]) ?>
    星期:<?= $this->callMacro('create_time_select', ['gsm[starttime][wday_start]', $config['time_start']['wday_start'], $config['time_array']['wdayArray']]) ?>
    时间:<?= $this->callMacro('create_time_select', ['gsm[starttime][hour_start]', $config['time_start']['hour_start'], $config['time_array']['hourArray']]) ?>:
    <?= $this->callMacro('create_time_select', ['gsm[starttime][minute_start]', $config['time_start']['minute_start'], $config['time_array']['minuteArray']]) ?>
  </td>
    <td>设置任务的开始呼叫时间</td>
  </tr>
  <tr>
    <th>结束时间：</th>
    <td>年:<?= $this->callMacro('create_time_select', ['gsm[endtime][year_end]', $config['time_end']['year_end'], $config['time_array']['yearArray']]) ?>
    月:<?= $this->callMacro('create_time_select', ['gsm[endtime][month_end]', $config['time_end']['month_end'], $config['time_array']['monthArray']]) ?>
    日:<?= $this->callMacro('create_time_select', ['gsm[endtime][mday_end]', $config['time_end']['mday_end'], $config['time_array']['mdayArray']]) ?>
    星期:<?= $this->callMacro('create_time_select', ['gsm[endtime][wday_end]', $config['time_end']['wday_end'], $config['time_array']['wdayArray']]) ?>
    时间:<?= $this->callMacro('create_time_select', ['gsm[endtime][hour_end]', $config['time_end']['hour_end'], $config['time_array']['hourArray']]) ?>:
    <?= $this->callMacro('create_time_select', ['gsm[endtime][minute_end]', $config['time_end']['minute_end'], $config['time_array']['minuteArray']]) ?>
  </td>
    <td>设置任务的结束呼叫时间</td>
  </tr>
</tbody>
